 ...
 */
class Spectrumted_Recipe_Adminhtml_Spectrumted_Recipe_PostController extends Spectrumted_Recipe_Controller_Adminhtml_Abstract {

    public function __construct($request, $response) {
        $this->_model = "spectrumted_recipe/post";
    ...
    private $_recipePostData = null;
    private function _getRecipePostData() {
        if (!$this->_recipePostData){
			...
			$imgPat = ""; 
					...
					$collection = Mage::getModel('spectrumted_recipe/post')->getCollection()->addFieldToFilter('entity_id', array('in' => $productIds));
					foreach($collection as $postt) {		
						$imgPat = ($postt->getImage()!='' ? $postt->getImage() : '');		
					}
				if($_FILES) {
					$data['image'] = ($_FILES['image']['name']!='' ? 'recipe/'.$_FILES['image']['name'] : $imgPat); 				
				}
				else {			
					$data['image'] = ($imgPat!='' ? $imgPat : '');
				}
            ...
            $this->_recipePostData = $data;
        }
        return $this->_recipePostData;
    }
    ...
    public function saveAction() {
        ...
        if ($data = $this->_getRecipePostData()) {
            ...
            try {
				...
				//$model->setMakeRecipe($data['make_recipe']);
				//echo "<pre>";
				//print_r($data);
				//print_r($model);
				//die;
				...
				if(isset($_FILES['image']['name'])) {
					...
					$uploader = new Varien_File_Uploader('image');
					 ...
					 $uploader->setFilesDispersion(false);
					  ...
					  $path = Mage::getBaseDir('media') . '/recipe/' ;
					
					 $uploader->save($path, $_FILES['image']['name']);
					
					 $data['image'] =$path. $_FILES['image']['name'];
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('spectrumted_recipe')->__('Post was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('spectrumted_recipe')->__('Unable to find post to save'));
        $this->_redirect('*/*/', array('store' => $storeId));
    }
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                $model = Mage::getModel('spectrumted_recipe/post');
            ...
            try {
                foreach ($postIds as $postId) {
                    ...
                    $post->setId($this->getRequest()->getParam('id'))
                            ->delete();
                    // $image = Mage::getModel('spectrumted_recipe/post')->load($imageId);
                    // $image->delete();
                }
    ...
    public function gridAction() {
        $this->loadLayout();
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('spectrumted_recipe/adminhtml_post_grid')->toHtml()
        );
    }
