 ...
 */
class Spectrumted_Recipe_Block_Rss_Recipe
...
{	
    ...
    function getCategoryCollection(){
        if($this->_categoryCollection == NULL){
            ...
            $this->_categoryCollection =   Mage::getModel('spectrumted_recipe/category')->getCollection()
    ...
    protected function _toHtml()
    {
        ...
        $newurl = Mage::getSingleton('core/url')->getUrl('spectrumted_recipe');
        $title = Mage::helper('rss')->__('Recipe Posts');
        $rssObj = Mage::getModel('rss/rss');
        $data = array('title' => $title,
                'description' => $title,
                'link'        => $newurl,
                'charset'     => 'UTF-8',
                'generator'   => 'Spectrumted Recipe Module http://www.spectrumted.com/',
        ...
        $detailBlock = Mage::getBlockSingleton('spectrumted_recipe/rss_post_details');

        Mage::dispatchEvent('rss_recipe_new_collection_select', array('collection' => $collection));
