 ...
 */
class Spectrumted_Recipe_Model_Attribute_Source_Comment_Type
extends Spectrumted_Recipe_Model_Attribute_Source_Abstract
{
    ...
    public function getAllOptions()
    {
        if (is_null($this->_options)) {
            $this->_options = array(
			array(
			'value'     => Spectrumted_Recipe_Helper_Data::XPATH_COMMENT_TEMPLATE_DEFAULT,
			'label'     => Mage::helper('spectrumted_recipe')->__('Customer Only - No Approval'),
			),
			array(
			'value'     => 'spectrumted/recipe/comment/form/facebook.phtml',
			'label'     => Mage::helper('spectrumted_recipe')->__('Facebook Comments'),
			),
		);
        }
