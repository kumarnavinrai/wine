 ...
 */
class Spectrumted_Recipe_Model_Resource_Category_Collection 
extends Spectrumted_Recipe_Model_Resource_Abstract_Store_Collection
{
    function _construct()
    {
    ...
    function addPopulatedOnlyFilter(){
        ...
        // $cond .= "AND posts.status = " . Spectrumted_Recipe_Model_Post::STATUS_ACTIVE;
        //foreach($this->_statuses as $status){
            $cond .= "AND posts.status IN (".implode(",",$this->_statuses).") ";
        //}
        if ($this->_storeFilterFlag){
            $cond .= " AND (FIND_IN_SET(".$this->_storeFilterId.", posts.store_ids) OR FIND_IN_SET(".Spectrumted_Recipe_Model_Abstract_Store::DEFAULT_STORE_ID.", posts.store_ids))";
        }
