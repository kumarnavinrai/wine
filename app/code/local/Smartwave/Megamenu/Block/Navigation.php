...
class Smartwave_Megamenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawMegaMenuItem($category,$mode = 'dt', $level = 0, $last = false)
    {
        ...
        if ($drawPopup) {
            ...
            $html[] = '<a href="'.'#'.'">'.$cat_icon.$name.$catLabel.'</a>';//$this->getCategoryUrl($category)
            if ($mode != 'mb') {
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                ...
                if ($level == 1) {
					...
					//version 1.0.2 end  
					 $parID = $child->getId();
						if($parID == 102 || $parID == 103 || $parID == 104)
						{ $html.= '<a class="level' . $level . '" style="color: darkolivegreen;font-size: 16px;" href="' . $this->getCategoryUrl($child) .  '"><span>' . $name .$label . '</span></a>'; }
						else
				   ...
				   $parID = $child->getParentId();
						if($parID == 79 || $parID == 91 || $parID == 105)
						{
							if($ii<9)
							{
								$html .= '<li class="'.$class.' '.$active.'">';
							...
							{
								$html .= '<li class="'.$class.' '.$active.'">';
                    			$html .= '<a class="level' . $level . '" style="color: darkolivegreen;font-size: 14px;" href="' . Mage::getModel("catalog/category")->load($parID)->getUrl() . '"><span>' . 'View All' .  '</span></a>';
								break;
							 }
							$ii = $ii + 1;
						}
